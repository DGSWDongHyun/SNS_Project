...
class FirebaseDatabaseModule {
    companion object {
        ...
        suspend fun sendNotification() {
            val service = RetrofitClient.getInstnace().create(FCMSend::class.java)

            val jobSendNotification : Deferred<Boolean> = CoroutineScope(Dispatchers.Main).async {

                val jobToComplete = GlobalScope.async {
                  database.child("user").addChildEventListener(object : ChildEventListener {
                      override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                          val getData = snapshot.getValue(User::class.java)

                          service.postFCM(ReqDataJson(ReqData(), getData!!.deviceToken)).enqueue(object : retrofit2.Callback<ResData> {
                              override fun onResponse(call: Call<ResData>, response: Response<ResData>) {
                                  if (response.code() == 200) {
                                      if(getData!!.deviceToken == null) Log.d("TAG", "null Token") else Log.d("TAG", "${getData!!.deviceToken}")

                                      Log.d("TAG", "${getData!!.deviceToken} ${response.body()} ${response.message()}, ${response.code()}")
                                  }
                              }
